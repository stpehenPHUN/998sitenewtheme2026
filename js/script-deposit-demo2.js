document.addEventListener("DOMContentLoaded", () => {
    ...
    function initPaymentUI() {
        ...
        function recalcSummary() {
            ...
            const methodMaxPay = getMethodMaxDeposit(channel, payFrom); // pay currency
...
`;
        ...
        if (amountInput) {
            // ✅ 允许手动输入（desktop + mobile）
            amountInput.removeAttribute("readonly");
            amountInput.setAttribute("inputmode", "numeric"); // iOS 数字键盘
            amountInput.setAttribute("pattern", "[0-9]*");
            amountInput.autocomplete = "off";

            const sanitizeDigits = (s) => String(s || "").replace(/[^\d]/g, "");

            // 输入时：只保留数字 + 千分位（光标会跳到末尾；如果你要完美光标我也能再升级）
            ...
            // 失焦再算一次，防止 iOS 某些情况下 input 事件不完整
            amountInput.addEventListener("blur", () => recalcSummary());
        }
    }
    /* =========================================================
       5) STARTUP
